...
def simulate_measure(deviceList=[i for i in range(1, 10)],
    ...
    G_data = generate_data(device_list=deviceList, repeats=repeats, event_repeat=event_repeat)
    ...
    for j in range(repeats):
        for i, device in enumerate(deviceList):
            ...
            I_SD = generate_IV(G_target, V_SD)
    ...
    save_for_manual_plot(MasterDF, basePath, save=True)

    with open(basePath + '/comments.txt', 'a') as f:
        f.write('average values: \n' + dfa.to_string() + '\n \n' + 'measurement finished at ' + str(datetime.now()))

    plot_all(MasterDF, title=fileName, save=True, basepath=basePath, cutoff=0.01)
