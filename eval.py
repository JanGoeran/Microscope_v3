...
def plot_all(df, title = 'title', cutoff = 1E-5, plot_repeat=False, save = False, basepath = 'G:/Shared drives/Nanoelectronics Team Drive/Data/2021/Marta/test_folder'):
    #deviceList = df.device.unique()
    ...
    for i, device in enumerate(liveDevices):
        df1 = df[df['device'] == device]
        ax1.plot(df1[xtype], df1['G'], color=next(color), label=device, linestyle=linestyles[i % 4 - 1])
